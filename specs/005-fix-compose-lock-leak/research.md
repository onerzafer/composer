...
**Decision**: Thread the `AbortSignal` into `runPipeline` and call `signal.throwIfAborted()` at each phase boundary, **mandatorily immediately before the `commit` phase** (the only phase that mutates the workspace/outputs). If the budget expires while a stuck `await` is parked, the race already rejected and the lock is released; if that `await` *later* resolves and the pipeline resumes, the pre-commit `throwIfAborted()` makes it throw instead of writing — so there is **no half-written spec and no half-emitted file**, preserving Atomic Compose. Additionally, **once the pre-commit `throwIfAborted()` passes, the budget timer is disarmed (`clearTimeout`) and the race detached before `commit` runs**, so a timeout firing *during* the fast, bounded commit critical section can never produce the contradictory outcome of a `ComposeTimeoutError` returned alongside files that were actually written. Commit is therefore uninterruptible; the budget only ever fires before it (closes analyze finding U1).
...
- **Invariant** asserted once in `core/src/config/limits.ts`: `maxHoldMs ≥ maxComposeDurationMs + ttlMarginMs` (default `ttlMarginMs = maxComposeDurationMs / 2`, floor of a few seconds). `ttlMarginMs` is the **TTL safety gap** (FR-002) and is deliberately distinct from SC-003's **timeout-detection margin** (≤ 5 s — how soon after the budget the abort fires); the spec calls both "margin", but they are unrelated quantities and carry distinct names in code. If a user config violates the invariant, raise a clear `ComposerConfigError` (fail fast at resolve time, not mid-compose).
- **Defaults**: `maxComposeDurationMs = 120_000`, `maxHoldMs = 180_000`. A warm compose is sub-second, but a **cold** catalog compile (adapter-next via the `tsx` loader on a fresh workspace) can take tens of seconds (~30 s+ observed in this repo's e2e), so the budget is deliberately generous — its job is to never kill a *legitimate* compose; age-based reclaim (R1) is the real recovery lever. To meet SC-001's ≤ 60 s recovery target where composes are known-fast, tune both down (e.g. `30_000` / `60_000`). Documented as tunable in either direction.
