...
- [ ] T004 Implement `EffectiveLimits`, `DEFAULT_LIMITS` (`maxComposeDurationMs: 30000`, `maxHoldMs: 60000`, `ttlMarginMs = floor(maxComposeDurationMs/2)`), and `resolveLimits(projectRoot, env = process.env)` with the `maxHoldMs ≥ maxComposeDurationMs + ttlMarginMs` invariant (throws `ComposerConfigError` naming both values + active env override) in `packages/core/src/config/limits.ts` (NEW).
